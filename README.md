...
Under a strict held-out protocol, one-shot generation fails on brand-critical text. A second edit step materially improves fidelity without structural regression. The evaluator proves both claims with evidence.
...
| What worked?    | A second edit step using only allowed references                                    |
...
| Second edit step | B     | 0.824   | 0.429 | 0.641 | 0.000     |

The edit recovered BACARDI label text (0.000 -> 0.429), improved brand score by +0.267, and reached 0.824 overall, with zero AFV regression and positive depth deltas across all 8 runs.
...
- Open `Strict Edit` to see the second-step edit result
...
4. **Edit**. A second edit step using only `label_closeup` and `side_left` recovers label text.
5. **Re-evaluate**. The evaluator confirms the improvement and checks for structural regression.

This is not pure one-shot generation. It's an evaluator-guided pipeline that treats generation failures as inputs to a second edit step and uses the evaluator to prove the fix worked.

## Supporting experiment history

These runs are included to show the iteration path that led to the final strict benchmark. The canonical path through the project is the strict baseline plus the strict edit step.
...
| Held-out edit                       | 8     | $0.24     | Non-leaking second edit step works, but still relies on a hero-angle reference |
...
In replay mode, start with the `Showcase` tab. It opens on the strict benchmark story with a curated baseline failure and strict edit success.
